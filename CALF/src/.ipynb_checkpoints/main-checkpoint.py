...
def main(args):
    ...
    train_gameList , val_gameList = train_test_split(games,test_size=0.2, random_state=42)

    # Create Train Validation and Test datasets
    if not args.test_only:
        dataset_Train = SoccerNetClips(path=args.SoccerNet_path, features=args.features, split="train", games=train_gameList, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate, chunks_per_epoch=args.chunks_per_epoch)
        dataset_Valid = SoccerNetClips(path=args.SoccerNet_path, features=args.features, split="valid", games=val_gameList, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate, chunks_per_epoch=args.chunks_per_epoch)
        dataset_Valid_metric  = SoccerNetClipsTesting(path=args.SoccerNet_path, features=args.features, split="valid", games=val_gameList, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate)
