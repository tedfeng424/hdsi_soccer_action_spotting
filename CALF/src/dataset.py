...
class SoccerNetClips(Dataset):
    def __init__(self, path, features="ResNET_PCA512.npy", split="train", games=None, 
        ...
        logging.info("Checking/Download features and label s locally")
        download_path = "/data/ted/hdsi/soccernet"
        downloader = SoccerNetDownloader(download_path)
        ...
        for game in tqdm(self.listGames):
            # Load features
            path_sn = "/data/ted/hdsi/soccernet"
            feat_half1_sn = np.load(os.path.join(path_sn, game, "1_ResNET_TF2_PCA512.npy"))
            feat_half2_sn = np.load(os.path.join(path_sn, game, "2_ResNET_TF2_PCA512.npy"))

            # Load CLIP features
            feat_half1 = np.load(os.path.join(self.path, game, "1_720p", self.features))['arr_0']
            feat_half2 = np.load(os.path.join(self.path, game, "2_720p", self.features))['arr_0']

            # check size:
            if not feat_half1_sn.shape[0] == feat_half1.shape[0]:
                if feat_half1_sn.shape[0] > feat_half1.shape[0]:
                    feat_half1 = np.vstack((feat_half1, np.zeros((1, 1024))))
                else:
                    feat_half1 = feat_half1[:-1]

            if not feat_half2_sn.shape[0] == feat_half2.shape[0]:
                if feat_half2_sn.shape[0] > feat_half2.shape[0]:
                    feat_half2 = np.vstack((feat_half2, np.zeros((1, 1024))))
                else:
                    feat_half2 = feat_half2[:-1]

            # Load labels
            labels = json.load(open(os.path.join(download_path, game, self.labels)))
...
class SoccerNetClipsTesting(Dataset):
    def __init__(self, path, features="ResNET_PCA512.npy", split="test", games=None,
        ...
        if split == "challenge":
            ...
            downloader.downloadGames(files=[self.labels, f"1_{self.features}", f"2_{self.features}"], split=[split], verbose=False)
            # downloader.downloadGames(files=[self.labels], split=[split], verbose=False)
    ...
    def __getitem__(self, index):
        ...
        # Load features
        path_sn = "/data/ted/hdsi/soccernet"
        feat_half1_sn = np.load(os.path.join(path_sn, self.listGames[index], "1_ResNET_TF2_PCA512.npy"))
        feat_half2_sn = np.load(os.path.join(path_sn, self.listGames[index], "2_ResNET_TF2_PCA512.npy"))

        # Load CLIP features
        feat_half1 = np.load(os.path.join(self.path, self.listGames[index], "1_720p", self.features))['arr_0']
        feat_half2 = np.load(os.path.join(self.path, self.listGames[index], "2_720p", self.features))['arr_0']
        ...
        # Load labels
        download_path = "/data/ted/hdsi/soccernet"
        if os.path.exists(os.path.join(download_path, self.listGames[index], self.labels)):
            labels = json.load(open(os.path.join(download_path, self.listGames[index], self.labels)))
