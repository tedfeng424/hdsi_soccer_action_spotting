...
def main(args):
    ...
    train_gameList = getListGames("train")
    val_gameList = getListGames("valid")
    train_path = "/data/ted/hdsi/openclip"
    valid_path = "/data/ted/hdsi/valid_data"
    # Create Train Validation and Test datasets
    if not args.test_only:
        dataset_Train = SoccerNetClips(path=train_path, features=args.features, split="train", games=train_gameList, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate, chunks_per_epoch=args.chunks_per_epoch)
        dataset_Valid = SoccerNetClips(path=valid_path, features=args.features, split="valid", games=val_gameList, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate, chunks_per_epoch=args.chunks_per_epoch)
        dataset_Valid_metric  = SoccerNetClipsTesting(path=valid_path, features=args.features, split="valid", games=val_gameList, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate)
    
    split_to_test = "test"
    if args.challenge:
        split_to_test="challenge"
    #dataset_Test  = SoccerNetClipsTesting(path=args.SoccerNet_path, features=args.features, split=split_to_test, games=val_gameList, framerate=args.framerate, chunk_size=args.chunk_size*args.framerate, receptive_field=args.receptive_field*args.framerate)
    ...
    model = ContextAwareModel(weights=args.load_weights, input_size=1024, num_classes=dataset_Test.num_classes, chunk_size=args.chunk_size*args.framerate, dim_capsule=args.dim_capsule, receptive_field=args.receptive_field*args.framerate, num_detections=dataset_Test.num_detections, framerate=args.framerate).cuda()
