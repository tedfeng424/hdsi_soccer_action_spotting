...
class SoccerNetClips(Dataset):
    def __init__(self, path, features="ResNET_PCA512.npy", split=["train"], version=1, 
        ...
        print("listGames length:" + str(len(self.listGames)))
        ...
        for game in tqdm(self.listGames):
            clip_feat1 = np.load(os.path.join('combined_features', 'combined_'+game[15:].replace('/','_')+'_1.npy'))
            clip_feat2 = np.load(os.path.join('combined_features', 'combined_'+game[15:].replace('/','_')+'_2.npy'))
            # Load features
                                 
            feat_half1 = np.load(os.path.join(self.path, game, "1_" + self.features))
            feat_half1 = np.concatenate((feat_half1, clip_feat1), axis=0)
            feat_half1 = feat_half1.reshape(-1, feat_half1.shape[-1])
            
            feat_half2 = np.load(os.path.join(self.path, game, "2_" + self.features))
            feat_half2 = np.concatenate((feat_half2, clip_feat2), axis=0)
...
class SoccerNetClipsTesting(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load features
        clip_feat1 = np.load(os.path.join('combined_features', 'combined_'+self.listGames[index][15:].replace('/','_')+'_1.npy'))
        clip_feat2 = np.load(os.path.join('combined_features', 'combined_'+self.listGames[index][15:].replace('/','_')+'_2.npy'))
